...
const useCharacterProfileLogic = (data: CharacterDetailData) => {
  return useMemo(() => {
    ...
    const rawUnionGrade = data.union_data?.union_grade;

    const unionDisplay =
      rawUnionLevel > 0 && rawUnionGrade
        ? `${rawUnionLevel.toLocaleString()} (${rawUnionGrade})`
        : "0";
...
const DataUpdateTooltip = () => {
  return (
    <Popover>
      ...
      <PopoverContent side="bottom" align="end" className="w-80 p-3 opacity-95">
...
export const CharacterProfileCard = ({ data }: CharacterProfileCardProps) => {
  ...
  return (
    <article
      className="bg-card relative flex h-full max-h-111.5 w-full max-w-90 flex-col items-center gap-2 rounded-xs border p-4 md:max-w-none md:flex-1 md:self-start"
    ...
    >
      ...
      {data.character_image && (
        <Image
          ...
          width={96}
          height={96}
          className="dark:border-border box-content -scale-x-100 transform rounded-full border-2 border-gray-300 bg-[#EAEEF3] dark:bg-[#323232]"
        />
      )}
