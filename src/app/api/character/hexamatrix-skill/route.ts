...
export async function GET(req: Request) {
  ...
  try {
    ...
    const data = await nexonFetch<CharacterHexaMatrixSkill>(
      `/character/hexamatrix-skill?ocid=${ocidQ}`,
      { cache: "no-store" },
    );

    return NextResponse.json<ApiResponse<CharacterHexaMatrixSkill>>({ data });
  } catch (e: unknown) {
    try {
