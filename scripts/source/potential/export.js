(() => {
  const TARGETS = ["한벌옷 160", "한벌옷 180", "한벌옷 200"];
  ...
  function parseBlock(headerIndex, fallbackEquipLevelText) {
    ...
    const rarityIdx = headerCells.findIndex((v) => v === "잠재옵션 등급");
  ...
  function parseTarget(target) {
    const headerIndexes = trs
      ...
      .filter(({ tr }) => {
        ...
        return text.includes(target) && text.includes("잠재옵션 등급");
      })
      .map(({ idx }) => idx);
